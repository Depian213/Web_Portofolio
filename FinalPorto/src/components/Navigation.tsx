      ...
      <div className="max-w-7xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="md:hidden text-white p-2 hover:bg-[#473472]/20 rounded-lg transition-colors"
          >
        ...
        {isOpen && (
          <div className="md:hidden mt-4 pb-4 flex flex-col gap-4 border-t border-[#473472]/20 pt-4">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                onClick={() => setIsOpen(false)}
                className="text-gray-300 hover:text-[#87BAC3] transition-colors duration-300 py-2"
              >
                {item.name}
              </a>
            ))}
